...
const ShowLayer = () => {
	...
	//两列两层渲染
	const renderDeskColumn = useCallback((deskColumn: SideModuleSchema[], realIndex) => {
			...
			<>
				...
				>
					{deskColumn.map((item, index) => {
						...
						return <SideModule item={item} index={realIndex} key={`sideModule${realIndex}`} />;
					})}
				</View>
				<SideModule item={deskColumn[4]} index={realIndex} key={`desk${realIndex}`} />
			</>
		);
	}, []);

	//根据generatorMap渲染
	const renderLayers: (generatorMap: SideModuleSchema[]) => JSX.Element[] = (generatorMap) => {
		let tempMap: SideModuleSchema[] = Object.assign([], generatorMap);
		// console.log(generatorMap);
		if (activeLayer <= generatorMap.length) {
					...
					<React.Fragment key={`desk${outIndex}`}>
						...
						{flag &&
							tempMap.map((item, index) => {
								realIndex = index + (outIndex + 1) * outItem.length;
								return <SideModule item={item} index={realIndex} key={`normal${realIndex}`} />;
							})}
					</React.Fragment>
				);
			});
		...
		return tempMap.map((item, index) => {
			return <SideModule item={item} index={index} key={`sideModule${index}`} />;
		});
	};
	return (
		<View style={[styles.container, { transform: [{ scale }] }]}>
			<View style={[styles.sideModuleBox, { ...sideModuleBox }]}>{renderLayers(generatorMap)}</View>
			{useMemo(() => {
				return (
					<View
						style={{
					...
					>
						<Image source={common["bottom_point"]} resizeMode="cover" />
						<Image style={{ flex: 1 }} source={common["bottom"]} resizeMode="stretch" />
						<Image
							style={{
